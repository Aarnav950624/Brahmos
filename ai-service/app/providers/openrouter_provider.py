...
class OpenRouterLLM:
    ...
    async def complete_vision(
        ...
        *,
        ...
        """Multimodal completion — image + text. Identification only."""
        if not self.configured:
            raise ProviderUnavailableError(
                "openrouter",
                "OPENROUTER_API_KEY is not configured on the AI service.",
        ...
        try:
            ...
            logger.error(
                "OpenRouter vision HTTP error %s: %s",
                exc.response.status_code,
                exc.response.text[:400],
            )
            raise ProviderUnavailableError(
            ...
            logger.error("OpenRouter vision network error: %s", exc)
            raise ProviderUnavailableError(
                "openrouter",
                "Unable to reach OpenRouter.",
            ) from exc
        ...
        logger.info(
            "OpenRouter vision ok model=%s chars=%s",
            self._settings.openrouter_model,
            len(text),
        )
        return text

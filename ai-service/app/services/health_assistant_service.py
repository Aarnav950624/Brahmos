...
class HealthAssistantService:
    ...
    async def answer(
        ...
        summary, points, model_hint = await self._synthesize(
            q, sources, payload.patient_context, payload.locale
        )
        ...
        if self._llm.configured:
            try:
                return await self._synthesize_with_llm(
                    question, sources, patient_context, locale
                )
    ...
    async def _synthesize_with_llm(
        ...
        lang = (locale or "en").lower().strip()
        if lang == "hi":
            lang_rule = (
                "Write summary and key_points in Hindi (Devanagari script). "
                "Keep medical terms clear."
            )
        elif lang == "gu":
            lang_rule = (
                "Write summary and key_points in Gujarati (Gujarati script). "
                "Keep medical terms clear."
            )
        else:
            lang_rule = "Write summary and key_points in clear English."

        prompt = (
            "You are a warm HealNexus care companion for post-discharge support.\n"
            "Answer THIS patient message specifically. Do not repeat a generic "
            "checkup dump. If they asked about medicines, list only relevant names/times "
            "from context. If they asked about today, talk about today's tasks. "
            "If they greet or share feelings, acknowledge that first.\n"
            "Never invent hospitals, medicines, or lab results that are not in context.\n"
            f"Language: {lang_rule}\n"
            "Return ONLY valid JSON with keys: summary (string, 2-6 sentences, "
            "directly answering the message), "
            "key_points (array of 0-5 short strings specific to this question).\n"
            "Do not diagnose or prescribe. Do not append the same warning list "
            "unless they asked about danger signs.\n\n"
            f"Message: {question}\n\n"
            f"HealNexus database context:\n{db_block or '(none)'}\n\n"
            f"Sources:\n{source_block}"
        )
        raw = await self._llm.complete(prompt, temperature=0.5, max_tokens=550)

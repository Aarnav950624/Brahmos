...
export function usePatientMutations() {
  ...
  const markMedicine = useMutation({
    mutationFn: ({
      ...
      slot?: string;
    }) => patientRepository.markMedicine(userId, medicineId, status, slot),
    onSuccess: async (_, vars) => {
      await invalidateAll();
      const when = vars.slot ? ` (${vars.slot})` : "";
      toast.success(
        vars.status === "taken"
          ? `Marked as taken${when}`
          : vars.status === "late"
            ? `Marked as late taken${when} — scores updated`
            : `Marked as skipped${when}`,
      );
    },
  });

  const reportMedicineUnavailable = useMutation({
    mutationFn: (medicineId: string) =>
      patientRepository.reportMedicineUnavailable(userId, medicineId),
    onSuccess: async (res) => {
      await invalidateAll();
      toast.success(
        res.already_open
          ? "Your health worker already has this medicine gap"
          : "Health worker notified — do not change the medicine yourself",
        { description: res.medicine_name },
      );
    },
  ...
  const updateCheckIn = useMutation({
    mutationFn: ({
      ...
      checkInId: string;
      input: CheckInInput;
    }) => patientRepository.updateCheckIn(userId, checkInId, input),
    onSuccess: async () => {
      await invalidateAll();
      await invalidateCareGraph(qc);
      void qc.invalidateQueries({ queryKey: ["doctor"] });

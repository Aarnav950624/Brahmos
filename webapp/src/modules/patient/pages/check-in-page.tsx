...
import { getStore, subscribeStore, type CheckInRow } from "@/data/store";
...
export function CheckInPage() {
  ...
  const { submitCheckIn, updateCheckIn } = usePatientMutations();
...
function PastCheckInEditor({
  ...
  busy: boolean;
  onCancel: () => void;
  onSave: (values: CheckInSchema) => Promise<void>;
}) {
  const form = useForm<CheckInSchema>({
    resolver: zodResolver(checkInSchema) as never,
    defaultValues: {
      ...
      medicine_taken: row.medicine_taken ?? true,
      notes: row.notes || "",
    },
  });
  const symptoms = form.watch("symptoms") || [];
  ...
  return (
    <form
      className="space-y-3 rounded-xl border border-primary/25 bg-card p-3"
    ...
    >
      <p className="text-xs font-medium text-muted-foreground">
        Editing {new Date(row.recorded_at).toLocaleString()}
      </p>
      <div className="grid gap-2 sm:grid-cols-2">
        <Field label="Systolic BP" id={`e-${row.id}-sys`}>
          <Input type="number" {...form.register("bp_systolic")} />
        </Field>
        <Field label="Diastolic BP" id={`e-${row.id}-dia`}>
          <Input type="number" {...form.register("bp_diastolic")} />
        </Field>
        <Field label="Blood sugar" id={`e-${row.id}-sugar`}>
          <Input type="number" {...form.register("blood_sugar")} />
        </Field>
        <Field label="Weight (kg)" id={`e-${row.id}-wt`}>
          <Input type="number" step="0.1" {...form.register("weight")} />
        </Field>
        <Field label="Pain (0–10)" id={`e-${row.id}-pain`}>
          <Input type="number" min={0} max={10} {...form.register("pain_score")} />
        </Field>
        <Field label="Sleep hours" id={`e-${row.id}-sleep`}>
          <Input type="number" step="0.5" {...form.register("sleep_hours")} />
        </Field>
      </div>
      <Field label="Mood" id={`e-${row.id}-mood`}>
        <select
          className="flex h-10 w-full rounded-xl border border-input bg-card px-3 text-sm"
          {...form.register("mood")}
        >
      ...
      <Field label="Notes" id={`e-${row.id}-notes`}>

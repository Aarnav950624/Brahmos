...
export type DoseSlotStatus = "pending" | "completed" | "late" | "skipped";

export function canonicalDoseSlots(slots: string[]): string[] {
  const normalized = normalizeSlots(slots);
  return normalized.length ? normalized : ["Morning"];
}

/** Map a dose label (Morning / 08:00) onto the four daily periods. */
export function periodFromSlot(
  slot: string,
): "morning" | "afternoon" | "evening" | "night" {
  const [canonical] = normalizeSlots([slot]);
  if (canonical === "Morning") return "morning";
  if (canonical === "Lunch") return "afternoon";
  if (canonical === "Dinner") return "evening";
...
export function eventStatusToDose(
  status: "taken" | "late" | "skipped" | "missed" | undefined,
): DoseSlotStatus {
  if (status === "taken") return "completed";
  if (status === "late") return "late";
  if (status === "skipped" || status === "missed") return "skipped";
  return "pending";
}

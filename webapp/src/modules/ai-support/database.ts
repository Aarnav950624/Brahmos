...
function detectTopic(question: string): AiDbTopic {
  ...
  if (
    /specialist|referr|who is my doctor|my (assigned )?doctor|care team|cardiolog|endocrin/.test(
      q,
    )
  )
    return "doctors";
...
export function shouldPreferDatabase(topic: AiDbTopic): boolean {
  // Registry facts only. Conversation, education, and “explain” go through
  // the AI service (still grounded with patient_context).
  return topic === "hospitals" || topic === "pmjay" || topic === "benefits";
}

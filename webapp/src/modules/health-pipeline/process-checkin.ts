...
function mapReadmissionLevel(
  ...
  if (category === "moderate" || category === "medium") return "moderate";
  return "low";
}

function persistClinicalRisk(patientId: string, now: string) {
  const live = computeClinicalRisk(patientId);
  updateStore((draft) => {
    const recovery = draft.recoveryScores.find((r) => r.patient_id === patientId);
    if (recovery) {
      recovery.score = live.recovery_score;
      recovery.computed_at = now;
    } else {
      draft.recoveryScores.push({
        patient_id: patientId,
        score: live.recovery_score,
        computed_at: now,
      });
    }
    const risk = draft.risks.find((r) => r.patient_id === patientId);
    if (risk) {
      risk.score = live.score;
      risk.level = live.level;
      risk.computed_at = now;
    } else {
      draft.risks.push({
        patient_id: patientId,
...
export function syncScoresFromEngine(
  ...
  const now = new Date().toISOString();
  const live = persistClinicalRisk(patientId, now);

  void persistPredictions(patientId, live.recovery_score, live.level, live.score);
  return {
    health,
    recovery_score: live.recovery_score,
    risk_level: live.level,
    risk_score: live.score,
  };
}
...
export async function processCheckInPipeline(
  ...
  const now = new Date().toISOString();
  const live = persistClinicalRisk(patientId, now);
  const recoveryScore = live.recovery_score;
  const riskLevel = live.level;
  const riskScore = live.score;

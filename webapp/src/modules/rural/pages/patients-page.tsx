...
import { cardFromPassport, loadOfflineHealthCard } from "@/modules/identity/offline-health-card";
      ...
      <div className="space-y-3">
        ...
        {list.map((p) => (
          ...
          >
            ...
            <OfflineCardPeek patientId={p.id} />
          </div>
        ))}
      </div>
    </div>
...
function OfflineCardPeek({ patientId }: { patientId: string }) {
  let card = loadOfflineHealthCard(patientId);
  if (!card) {
    const live = identityRepository.getDigitalPassport(patientId);
    if (live) card = cardFromPassport(live, live.preferred_language);
  }
  if (!card) {
    return (
      <p className="mt-2 text-xs text-muted-foreground">
        Offline card not cached yet — open the patient passport once while
        online.
      </p>
    );
  }
  return (
    <div className="mt-3 rounded-2xl border border-emerald-200 bg-emerald-50/60 px-3 py-2 text-xs">
      <p className="font-semibold text-emerald-900">Offline health card</p>
      <p className="mt-1 text-muted-foreground">
        Blood {card.blood_group || "—"} · Allergies{" "}
        {card.allergies.join(", ") || "none"} · Meds{" "}
        {card.medicines.map((m) => m.name).join(", ") || "—"}
      </p>
      <p className="mt-1 text-muted-foreground">
        Emergency {card.emergency_contact?.name || "—"}{" "}

...
import {
  cachedPublicPassport,
  decodePublicPassport,
  encodePublicPassport,
} from "@/modules/identity/public-passport";
import type {
  BenefitsDashboard,
  DigitalPassport,
  EmergencyProfile,
...
export const identityRepository = {
  ...
  getDigitalPassport(userOrPatientId: string): DigitalPassport | null {
    ...
    const result: DigitalPassport = {
      ...
      address_city:
        ...
        null,
      preferred_language: patient.preferred_language || profile.locale || "en",
    };

    try {
      saveOfflineHealthCard(
        cardFromPassport(result, result.preferred_language),
      );
    } catch {
      /* quota / private mode */
    }
  ...
  getEmergencyProfile(
    ...
    if (patientId) {
      const live = this.getEmergencyProfileByPatient(patientId);
      encodePublicPassport(live);
      return live;
    }
    if (token.toUpperCase().includes("DEMO")) {
      const demo = this.getEmergencyProfileByPatient(IDS.patient);
      encodePublicPassport(demo);
      return demo;
    }
    ...
    if (byPatient) {
      const live = this.getEmergencyProfileByPatient(byPatient.id);
      encodePublicPassport(live);
      return live;
    }
    const fromUrl = decodePublicPassport(packedFromUrl);
    if (fromUrl) return fromUrl;
    return cachedPublicPassport(token);
  },
  ...
  emergencyQrUrl(token: string, packed?: string | null): string {
    const path = `/emergency/${encodeURIComponent(token)}`;
    if (typeof window === "undefined") {
      return packed ? `${path}?c=${packed}` : path;
    }
    const origin = window.location.origin;
    const card =
      packed ||
      (() => {
        const live = this.getEmergencyProfile(token);
        return live ? encodePublicPassport(live) : null;
      })();
    return card ? `${origin}${path}?c=${card}` : `${origin}${path}`;
  },

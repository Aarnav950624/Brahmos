...
import { periodFromSlot } from "@/modules/medicines/repository";
...
function buildMedicines(patientId: string, meds: MedicineRow[]): MedicineDose[] {
  ...
  meds.forEach((med, mi) => {
    ...
    slots.forEach((slot, si) => {
      const slotted = events.some(
        (e) => e.medicine_id === med.id && e.scheduled_for,
      );
      const ev = slotted
        ? events.find(
            (e) => e.medicine_id === med.id && e.scheduled_for === slot,
          )
        : events.find((e) => e.medicine_id === med.id);

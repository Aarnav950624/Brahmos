import { ChevronDown, ChevronsLeft, LogOut, Menu, Moon, Sun } from "lucide-react";
...
export function Navbar({ title }: NavbarProps) {
  ...
  const { setMobileOpen, toggleCollapsed, sidebarCollapsed } = useShell();
    ...
    <header className="sticky top-0 z-30 border-b border-border/80 bg-card/90 backdrop-blur-xl">
      <div className="flex h-14 items-center justify-between gap-3 px-4 md:px-5">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <button
            type="button"
            className="hidden h-8 w-8 items-center justify-center rounded-full border border-border/80 bg-card text-muted-foreground shadow-sm transition hover:border-primary/30 hover:bg-primary/5 hover:text-foreground md:inline-flex"
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <div className="relative" ref={menuRef}>
            <button
              ...
              className="flex items-center gap-2 rounded-xl border border-border/80 bg-background py-0.5 pl-0.5 pr-2 transition hover:border-primary/30 sm:pr-2.5"
              aria-haspopup="menu"

...
export async function askHealthAssistant(
  ...
  const locale =
    extras?.locale === "hi" || extras?.locale === "gu" ? extras.locale : "en";

  if (!isAiServiceConfigured()) {
    return {
      summary:
        extras?.local_summary ||
  ...
  try {
    ...
    return {
      ...
      key_points: [
        "Use your care plan meanwhile",
        "Contact your clinician if urgent",
      ],
      when_to_contact_doctor: [
        "Chest pain, severe breathlessness, confusion, or fainting",
      ],
...
export async function fetchVisitBrief(payload: Record<string, unknown>) {
  if (!isAiServiceConfigured()) return null;
  return aiRequest<Record<string, unknown>>("/ai/visit-brief", {
    method: "POST",
    body: payload,
  });
}

export interface MedicineExtractApiResult {
...
/**
 * Identify medicine packaging text from a temporary photo.
 * Identification / support only — never a prescription.
 */
export async function extractMedicineFromImage(input: {
  imageBase64: string;
  mimeType?: string;
}): Promise<MedicineExtractApiResult> {
  if (!isAiServiceConfigured()) {
    throw new ApiError("AI API base URL is not configured", 503);
  }
  return aiRequest<MedicineExtractApiResult>("/ai/medicine/extract", {
    method: "POST",
    body: {
